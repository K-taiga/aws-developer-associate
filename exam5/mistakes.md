...
* recipeは全体の構成、スタックはクラスターみたいなものでそのコンポーネントがレイヤー
...
## 質問63:不正解
>A社は顧客管理向けのJavaアプリケーションをAWSを利用して構築しています。WEBサーバーにEC2インスタンスを利用して、データベースシステムにRDSを使用し、顧客の構成情報データを蓄積します。またKMSを使用してデータを暗号化することが要件になっています。暗号化に使用するデータキーの暗号化されたコピーのみを返すAPIを呼び出す必要があります。また、Decrypt APIを呼び出してデータキーを復号化し、返されたキーを使用して最終的にデータを暗号化します。

>この要件を満たすために適切なKMS APIを選択してください。
...
* GenerateDataKeyWithoutPlaintextはリクエストごとに一意のデータキーを生成しそのコピーのみを返す コピーを返すという点以外はGenerateDataKeyと同じ

* そのコピーを使ってDecryptを行う

## 質問64:不正解
>B社ではWEBアプリケーションのデータベースとしてRDS MySQLを利用しています。現在このMySQLデータベースが応答しなくなる障害が発生し、あなたは対応に追われています。 まず初めにトラブルシューティングのために実行に時間がかかった全てのSQLステートメントを収集することになりました。

>全てのSQLステートメントを収集する最適な方法はどれでしょうか。
...
* MySQLはエラーログ、スロークエリログ、一般ログをモニタリングできる
* エラーログはデフォルトで生成され、他２つはパラメータの設定によって生成できる
* スロークエリログは実行にlong_query_time秒以上かかるSQLステートメントで構成されるもので、これを有効にしてmin_examined_row_limitを調べることがまず必要

## 質問65:不正解
>A社はLambda関数とAPI Gatewayを統合した新しいサーバーレスアーキテクチャを実装しています。Lambda関数は、Aurora MySQL DBインスタンスにデータを送信しますが、着信トラフィックが急増するたびに「エラー1040」という接続エラーが多発してしまいます。 Lambda関数が呼び出されるたびに常に新しいデータベース接続を実施していることが原因のようです。

>この問題に対応するために、最適な実装方式を選択してください。（2つ選択してください。）

*A: Lambda関数で実行コンテキストを使用し、コードにロジックを追加して、接続前に接続が存在するかどうかを確認する*
*RDSプロキシを利用する*

***
note:

* 実行コンテキストを再利用することで、このコンテキストでDBへの接続を行っている場合その接続を次の処理でも維持できる
* Lambda関数の完了後にサービスは実行コンテキストをフリーズし、再利用のためにコンテキストを解凍ということを行う
* Lambdaは実行されるたびにコネクションが増えエラーになるため、RDSプロキシで接続を確立する